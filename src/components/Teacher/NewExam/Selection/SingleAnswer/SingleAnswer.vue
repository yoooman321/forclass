    ...
    <div class="option-row" v-for="(option, selectionIndex) in options" :key="`${selectionIndex}-${option}`">
      ...
      <q-item tag="label" v-ripple class="option">
        <q-item-section avatar>
          <q-checkbox :value="option.isAnswer" @input="e => changeOptionisAnswer(selectionIndex, e)" color="teal"/>
        </q-item-section>
        <q-item-section>
          <q-file
        ...
        <q-btn round color="grey-9" icon="close" @click="deleteOption([index, selectionIndex])" class="deleteBtn"></q-btn>
      </q-item>
    </div>
...
export default {
  ...
  methods: {
    ...
    changeOptionisAnswer (index, e) {
      this.$store.dispatch('changeOptioniSingleAnswer', { questionIndex: this.index, selectionIndex: index, value: e })
    },
